...
struct YourApp: App {
    init() {
        // Synchronous, and it must run before anything can purchase.
    ...
    var body: some View {
        ...
        .checkoutSheet(item: $selectedProduct) { result in
            ...
            case .failure(let error):
                // ZeroSettleError is not Equatable — use isCancellation(_:),
                // which also catches CancellationError and StoreKit's own cancel.
...
`PaymentIntent` results are cached for 30 minutes. This means re-opens after dismiss skip the API call entirely. The cache is invalidated automatically after a successful payment.
...
Choose a user ID that won't change. See [Picking an identity on iOS](/iap/user-identity#picking-an-identity-on-ios) for the full rundown.

<CodeGroup>
```swift Swift
// Best on iOS: Sign in with Apple's opaque user identifier.
// Stable forever, scoped to your team, same on every device the person
// signs into, survives reinstall — and scoped to the Apple ID that owns
// their StoreKit purchases.
...
All SDK errors are `ZeroSettleError` cases. [Checkout Errors](/iap/checkout-errors) is the complete reference — every case, its cause, and the recommended handling. The ones you will hit most:

| Error Case | When It's Thrown |
|---|---|
| `.notConfigured` | SDK not configured before use |
| `.productNotFound(String)` | Product ID not found in catalog |
| `.cancelled` | User dismissed the checkout |
| `.apiError(APIErrorDetail)` | Network or server error — read `statusCode` / `serverMessage` |
| `.checkoutFailed(reason:)` | Payment processing failed |
...
<Note>
  `ZeroSettleError` is not `Equatable` — several cases carry an `any Error` payload. Match with `if case` / `switch`, and use `ZeroSettleError.isCancellation(_:)` rather than `== .cancelled`.
</Note>
...
.checkoutSheet(item: $selectedProduct) { result in
    ...
    case .success(let transaction):
        unlockProduct(transaction.productId)
    case .failure(let error):
        // Cancellations are not errors — filter them before any UI.
        if ZeroSettleError.isCancellation(error) { return }

        if let zsError = error as? ZeroSettleError {
            switch zsError {
            case .notConfigured:
                // Misconfiguration — log it and hide the purchase path.
                // Don't crash a shipping app on a payment failure.
                logger.error("ZeroSettle not configured")
                hidePurchaseUI()
            case .productNotFound(let id):
                logger.error("Product \(id) not found in the catalog")
                hidePurchaseUI()
            case .apiError(let detail):
                showRetry(detail.serverMessage ?? "Something went wrong. Please try again.")
...
<Tip>
  `warmUp()` caches the `PaymentIntent` for 30 minutes, matching the backend's checkout-config expiry. If the user reaches the paywall within that window, the sheet opens without any network delay.
</Tip>

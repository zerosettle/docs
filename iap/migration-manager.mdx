...
<Warning>
  **`ZSMigrationManager` is deprecated.** Use [ZSOfferManager](/iap/offer-manager) instead — it is a strict superset that handles migration, StoreKit-to-web upgrades, and web-to-web upgrades from a single manager, with unified demo mode and server-driven checkout presentation. This page is retained for existing integrations; new code should use `ZSOfferManager`.
</Warning>

`ZSMigrationManager` is the state machine behind `MigrationTipView` (itself deprecated in favor of `OfferTipView`). If the built-in view doesn't fit your design, use the manager directly to build a fully custom switch and save experience while reusing all the eligibility logic, checkout orchestration, and state tracking.

## Quick Start

<Note>
  Call `ZeroSettle.shared.identify(_:)` once at app launch before constructing the manager — see the [Quickstart](/iap/quickstart). `migrationManager()` itself is deprecated; prefer `ZeroSettle.shared.offerManager()`.
</Note>
...
<Tabs>
  <Tab title="SwiftUI">
    ...
    struct SwitchAndSaveBanner: View {
        // migrationManager() throws if identify(_:) hasn't been called — that's a programming error.
        ...
        // Or: try! ZeroSettle.shared.migrationManager(stripeCustomerId: "cus_abc123")
  ...
  <Tab title="UIKit">
    ...
    class SwitchAndSaveViewController: UIViewController {
        ...
        // Pass stripeCustomerId to attach checkouts to an existing Stripe customer.
        // migrationManager() throws if identify(_:) hasn't been called yet.
        init(stripeCustomerId: String? = nil) throws {
            self.manager = try ZeroSettle.shared.migrationManager(stripeCustomerId: stripeCustomerId)
...
### Forcing a jurisdiction (debug)

The `forceUSARegion()`, `resetRegionOverride()`, and `isUSARegionForced` static APIs are **deprecated**. Use the SDK-wide override on `ZeroSettle.shared` instead — it supports all jurisdictions and applies across every jurisdiction-sensitive API:

```swift
#if DEBUG
// Force the jurisdiction so the offer appears regardless of device locale
...
<Note>
Unlike `demoMode`, `forcedJurisdiction` only overrides the jurisdiction check. All other eligibility checks (active StoreKit subscription, rollout, tenure, etc.) still apply. Debug / preview only — gate assignments behind `#if DEBUG`.
</Note>
...
<Tabs>
  ...
  <Tab title="UIKit">
    ...
    class CustomSwitchAndSaveViewController: UIViewController {
        ...
        // migrationManager() throws if identify(_:) hasn't been called yet.
        init() throws {
            self.manager = try ZeroSettle.shared.migrationManager()
            super.init(nibName: nil, bundle: nil)
        }

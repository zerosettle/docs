...
<Tip>
  Not selling through ZeroSettle yet, and only want the StoreKit tracking? See [Analytics-Only Integration](/iap/analytics-only) — configure, identify, done.
</Tip>
...
## Optional: reporting a purchase your app made

<Note>
  Not needed if you purchase through the SDK. `purchaseViaStoreKit(productId:)`, `purchase(productId:)`, and `.checkoutSheet` all sync on your behalf, and renewals, refunds, and revocations arrive through the listener regardless.
</Note>

If your app calls StoreKit's `Product.purchase()` directly, that one transaction is not redelivered through `Transaction.updates` — Apple does not redeliver what `purchase()` already returned. It still reaches ZeroSettle at the next `identify(_:)`. To report it immediately instead:

```swift
Task { await ZeroSettle.shared.recordStoreKitPurchase(result) }
```

Non-throwing, does not finish the transaction, and retries through the same persistent queue as the listener. Requires ZeroSettleKit 1.6.0+. See [Analytics-Only Integration](/iap/analytics-only) for the full pattern.

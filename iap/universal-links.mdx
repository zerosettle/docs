...
class MainActivity : ComponentActivity() {
    override fun onNewIntent(intent: Intent?) {
        ...
        intent?.data?.let { ZeroSettle.handleDeepLink(it) }
    }
...
Linking.addEventListener('url', ({ url }) => ZeroSettle.handleUniversalLink(url));
```
...
Checkout results — successful purchases, cancellations, and errors — are handled via the `CheckoutSheet` result callback, not universal links. See [Checkout Errors](/iap/checkout-errors) for the complete result handling API and error reference.

...
<Warning>
  Always set `syncStoreKitTransactions: false` (iOS) or `syncPlayPurchases = false` (Android) when using RevenueCat. Both SDKs listening for store transactions will cause conflicts.
</Warning>

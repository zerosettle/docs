...
Initialize the SDK as early as possible in your app's lifecycle. On iOS, call `configure()` early, then `identify()` to set the active user and fetch products. On Android, `configure()` is synchronous and should be called in `Application.onCreate()`, then call `bootstrap()` to fetch products and restore entitlements:

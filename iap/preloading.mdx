...
3. The response (checkout URL, transaction ID, client secret) is cached in `CheckoutResponseCache` with a **30-minute TTL**, aligned to the backend's `checkout_config_expires_at`
...
- **SDK cache expires** (30 minutes) → the next checkout open creates a fresh PI; the old one is eventually cleaned up by Stripe
...
| PI caching (30-min TTL) | **ON** | Built-in, no config needed |
| Request coalescing | **ON** | Built-in, no config needed |
| WebView pre-rendering | **ON** (no limit) — but only runs once PI caching has fired for a product | Cap with `maxPreloadedWebViews` |

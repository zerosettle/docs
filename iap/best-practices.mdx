...
Initialize the SDK as early as possible in your app's lifecycle. On iOS, call `configure()` early, then `identify(_:)` with an `Identity` enum value (`.user(id:)`, `.anonymous`, or `.deferred`) to declare the active user and fetch products. On Android, `configure()` is synchronous and should be called in `Application.onCreate()`, then call `bootstrap()` to fetch products and restore entitlements:
...
<Note>
  Preloading and `warmUp()` are iOS-specific APIs. On iOS, products are cached after calling `identify(_:)`. On Android, products are cached in the `ZeroSettle.products` StateFlow after calling `bootstrap()` or `fetchProducts()`. No additional preloading step is needed.
</Note>
...
ZeroSettle.shared.identify(.user(id: user.id))  // "usr_abc123"

// Good: Firebase UID
ZeroSettle.shared.identify(.user(id: Auth.auth().currentUser?.uid ?? ""))

// Bad: Email (users can change it)
ZeroSettle.shared.identify(.user(id: user.email))

// Bad: Device identifier (changes on reinstall)
ZeroSettle.shared.identify(.user(id: UIDevice.current.identifierForVendor?.uuidString ?? ""))
```
...
When you call `identify(_:)` on iOS, products are fetched and the first product's `PaymentIntent` is warmed up automatically. No separate calls are needed.
...
struct HomeView: View {
    var body: some View {
        ...
        .task {
            // Products already fetched by identify(_:) — warm up additional products
            let products = ZeroSettle.shared.products

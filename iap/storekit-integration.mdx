...
## Reporting a purchase your app made

`Transaction.updates` does not redeliver a transaction StoreKit already returned from `Product.purchase()`. If your app runs its own StoreKit purchase rather than calling `purchaseViaStoreKit(productId:)`, report it explicitly:

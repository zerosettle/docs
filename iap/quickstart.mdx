        ...
        var body: some Scene {
            WindowGroup {
                ContentView()
                    ...
                    // `Auth.currentUser.id` stands in for YOUR auth system's user id
                    // (Firebase, your own backend, whatever you use). No user system
                    // yet? See Identifying Users for the anonymous option — read it
                    // first, the choice is hard to reverse later.
                    .task {
                        try? await ZeroSettle.shared.identify(.user(id: Auth.currentUser.id))
                    }
                    // Harmless on the default webview mode, and already in place if you
                    // ever switch to a Safari checkout mode. See Step 2.
                    .zeroSettleHandler()
            }
        }
    }
...
<Step title="Install the universal link handler (only for Safari checkout modes)">

<Check>
  **Skip this step for now if you are just getting to a first purchase.** New accounts
  default to `webview` checkout, which runs inside your app and returns the result
  directly — no universal link, no Associated Domains, no paid Apple Developer account,
  no physical device.
</Check>

If you switch your checkout mode to in-app Safari (`safari_vc`) or external Safari
(`safari`), the result comes back through a universal link instead, and you need a handler
to receive it. Because checkout mode is a server-side setting, add the handler before you
change modes — see [Universal Links](/iap/universal-links) for the full setup, which does
require a paid Apple Developer account and a physical device to test.
...
- **Sandbox:** `applinks:api.zerosettle.io?mode=developer` and `applinks:zerosettle.io`
- **Production:** `applinks:api.zerosettle.io` and `applinks:zerosettle.io`

Both domains are required. `api.zerosettle.io` handles the checkout callback; `zerosettle.io` handles the "Return to App" fallback. See [Universal Links](/iap/universal-links) for the full setup.

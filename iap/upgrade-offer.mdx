...
<Note>
  These APIs require an identified user. Call `ZeroSettle.shared.identify(_:)` (Swift) or `ZeroSettle.identify(...)` (Android) once at app launch — see the [Quickstart](/iap/quickstart). The Swift snippets below use the `userId`-less APIs that read the active user from `identify(_:)`.
</Note>
...
if config.available, let target = config.targetProduct {
    ...
    print("Not eligible: \(config.reason?.rawString ?? "unknown")")
}
```

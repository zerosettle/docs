...
Configure the SDK early in your app's lifecycle. `configure()` is synchronous. Then call `identify()` to set the active user, fetch products, and restore entitlements.
        ...
        WindowGroup {
            ContentView()
                .task {
                    ...
                    try? await ZeroSettle.shared.identify(userId: Auth.currentUser.id)
                }
...
After configuring, `identify()` automatically:
...
It is the canonical entry point for setting the active user — every other user-scoped API on `ZeroSettle.shared` reads from this identified user, so you never need to pass `userId` again. All automatic steps are non-fatal — failures are logged but won't prevent the SDK from working.

<Warning>
  **Don't skip `identify()`.** If you call `configure()` but never call `identify()`, StoreKit purchases that arrive via `Transaction.updates` won't be attributed to a user and will be left unfinished — they will not appear in your dashboard. The SDK logs a warning if no user is identified within 10 seconds of `configure()`.
</Warning>

<Note>
  **Migrating from `bootstrap()`?** `identify(userId:name:email:)` replaces `bootstrap(userId:name:email:)` as of SDK **1.2.4**. The signatures are identical and `bootstrap()` still works (it's deprecated and slated for removal in 2.0). All user-scoped methods that took a `userId` parameter now have a `userId`-less variant — see the [migration guide](/iap/migration-guide#sdk-1-2-4-identify) for the full list.
</Note>

...
<Note>
  If your code uses `ZSMigrateTipView`, it still works — it's a type alias for `MigrationTipView`.
  For new integrations supporting both migration and upgrade offers, see [Offer Tip View](/iap/offer-tip-view).
</Note>

`MigrationTipView` is a self-contained view that prompts eligible users to switch from Apple billing to direct billing at a discount. It manages its own visibility, checkout flow, and dismissal state. You just add it to your layout.
...
Switch & Save also works for **Google Play Billing** subscribers on Android. The user flow mirrors the StoreKit flow — the subscriber switches to discounted web billing, keeps Play access until their current period ends, and the old Play subscription is cancelled — with one key difference: after the web checkout succeeds, **ZeroSettle's backend cancels the Play subscription programmatically** via the Play Developer API (StoreKit can't be cancelled server-side, so Apple users have to do it themselves in iOS Settings).

<Steps>
  <Step title="Enable Google Pay in Stripe">
    Play → web migrations charge through Stripe, so your connected Stripe account must have **Google Pay** enabled. Turn it on under **Settings → Payment methods → Wallets** in your Stripe Dashboard. Without it, the payment sheet falls back to card entry on Android.
  </Step>
  ...
  <Step title="The backend cancels the Play subscription (best-effort)">
    On `payment_intent.succeeded`, ZeroSettle calls `subscriptionsv2.cancel` for the user's old Play subscription, with retries. If the cancellation can't be completed (e.g., Play returns a permanent error, or it keeps failing), the transaction is flagged for **manual intervention** instead of silently failing.
  </Step>

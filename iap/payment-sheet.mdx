...
<Tabs>
  <Tab title="SwiftUI">
    Use the `.checkoutSheet()` view modifier on any view. Pass an optional `ZSProduct?` binding — the sheet presents when the binding is non-nil and resets to `nil` on dismiss. This mirrors SwiftUI's `.sheet(item:)` pattern, and works for both single-product paywalls and multi-product stores.
